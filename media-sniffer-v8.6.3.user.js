// ==UserScript==
// @name         媒体嗅探器 Media Sniffer Pro v8.6.3
...
(function () {
    ...
    LANG.strings = {
        ...
        ,
            ...
            'cookie': 'Cookie:',
            'infoLine1': '媒体嗅探器 Pro v8.6.3 · 模块化架构 · AES-128 解密 · 虚拟列表 · 进度可视化 · 主题切换 · 滚动优化',
    ...
    var DEFAULT_CONFIG = {
        theme: 'auto',
        palette: 'indigo',         // 配色方案: indigo/purple/blue/green/orange/rose
    ...
    UI.registerPalette = function (id, def) {
        ...
        UI._palettes[id] = {
            ...
            light: def.light || { primary: '#6366f1', primary2: '#8b5cf6' },
            dark: def.dark || { primary: '#818cf8', primary2: '#a78bfa' },
        };
    ...
    UI.getPalette = function () {
        ...
        if (!id || !UI._palettes[id]) id = 'indigo';
        return id;
    };

    // 内部：取当前 palette 在当前明暗模式下的强调色
    UI._paletteColors = function () {
        var p = UI._palettes[UI.getPalette()] || UI._palettes['indigo'];
        return State.getTheme() === 'dark' ? p.dark : p.light;
    };
    ...
    // 注册内置预设配色
    UI.registerPalette('indigo', {
        nameKey: 'paletteIndigo',
        light: { primary: '#6366f1', primary2: '#8b5cf6' },
        dark:  { primary: '#818cf8', primary2: '#a78bfa' },
    });
    UI.registerPalette('purple', {
        nameKey: 'palettePurple',
        light: { primary: '#8b5cf6', primary2: '#a855f7' },
        dark:  { primary: '#a78bfa', primary2: '#c084fc' },
    });
    UI.registerPalette('blue', {
        nameKey: 'paletteBlue',
        light: { primary: '#3b82f6', primary2: '#06b6d4' },
        dark:  { primary: '#60a5fa', primary2: '#22d3ee' },
    });
    UI.registerPalette('green', {
        nameKey: 'paletteGreen',
        light: { primary: '#10b981', primary2: '#14b8a6' },
        dark:  { primary: '#34d399', primary2: '#2dd4bf' },
    });
    UI.registerPalette('orange', {
        nameKey: 'paletteOrange',
        light: { primary: '#f59e0b', primary2: '#f97316' },
        dark:  { primary: '#fbbf24', primary2: '#fb923c' },
    });
    UI.registerPalette('rose', {
        nameKey: 'paletteRose',
        light: { primary: '#f43f5e', primary2: '#ec4899' },
        dark:  { primary: '#fb7185', primary2: '#f472b6' },
    });
    ...
    UI.createToggle = function (initialState, onChange, color) {
        ...
        knob.style.cssText = 'position:absolute;top:2px;width:24px;height:24px;border-radius:50%;background:#fff;box-shadow:0 2px 6px rgba(0,0,0,.25);transition:transform .25s ease;';
        
        container.appendChild(knob);
        
        function setColor(c) {
            var dark = State.getTheme() === 'dark';
            var bgOff = dark ? '#334155' : '#e2e8f0';
    ...
    UI.renderSettings = function () {
        ...
        for (var ti = 0; ti < themes.length; ti++) {
            (function (t) {
                ...
                b.style.cssText = 'flex:1;min-width:90px;padding:10px;border:none;border-radius:10px;background:' + (active ? 'linear-gradient(135deg,' + c.primary + ',' + c.primary2 + ')' : c.bg3) + ';color:' + (active ? '#fff' : c.txt) + ';font-size:13px;font-weight:600;cursor:pointer;' + (active ? 'box-shadow:0 4px 12px ' + c.primary + '4d;' : '');
        ...
        palBox.style.cssText = 'padding:14px;border-radius:12px;background:' + c.bg2 + ';border:1px solid ' + c.border + ';margin-bottom:14px;';
        palBox.innerHTML = '<div style="font-size:13px;font-weight:600;color:' + c.txt + ';margin-bottom:10px;">🎨 ' + LANG.t('paletteSelect') + '</div>';
        var palRow = document.createElement('div');
        palRow.style.cssText = 'display:flex;gap:10px;flex-wrap:wrap;';
        ...
        for (var pi = 0; pi < palettes.length; pi++) {
            (function (pal) {
                ...
                item.style.cssText = 'flex:1;min-width:64px;cursor:pointer;text-align:center;';
                ...
                label.style.cssText = 'margin-top:6px;font-size:11px;color:' + (active ? c.primary : c.sub) + ';font-weight:' + (active ? '600' : '400') + ';';
        ...
        updBox.style.cssText = 'background:' + c.bg2 + ';border-radius:10px;padding:12px;margin-bottom:12px;';
        ...
        updBox.appendChild(updBtnRow);
        container.appendChild(updBox);
